...
function initTableFromUserData() {
    ...
    if (currentUserData) { // Verifică dacă s-a găsit utilizatorul curent
        ...
        properties.forEach((property, index) => {
            ...
            <td>
                <button class="btn-favorite" onclick="addToFavorites('${property.city}', '${property.streetName}', '${property.streetNumber}')" data-city="${property.city}" data-street="${property.streetName}" data-number="${property.streetNumber}">&#9734;</button>
...
function addToFavorites(city, streetName, streetNumber) {
    ...
    userDataArray.forEach(user => {
        ...
        properties.forEach(property => {
            if (property.city === city && property.streetName === streetName && property.streetNumber === streetNumber) {
                ...
                if (property.favorite) {
                    ...
                    favorites = favorites.filter(favProperty => !(favProperty.city === city && favProperty.streetName === streetName && favProperty.streetNumber === streetNumber));
                }
...
function updateFavoriteButton(city, streetName, streetNumber, isFavorite) {
    ...
    buttons.forEach(button => {
        if (button.dataset.city === city && button.dataset.street === streetName && button.dataset.number === streetNumber) { // Verifică dacă butonul corespunde proprietății specificate
            if (isFavorite) {
...
function favoriteslist() {
    ...
    if (favorites.length > 0) { // Verifică dacă există proprietăți favorite
        ...
        favorites.forEach(property => {
            ...
            <td>${property.hasAC ? 'Yes' : 'No'}</td> 
            <td>
                <button class="btn-delete" onclick="removeFromFavorites('${property.city}', '${property.streetName}', '${property.streetNumber}')">Remove</button>
            </td>
            `;
...
function updateFavoriteButtonsStyle(favorites) {
    let buttons = document.querySelectorAll('.btn-favorite'); // Obține toate butoanele de favorite
    buttons.forEach(button => {
        let isFavorite = favorites.some(property => button.dataset.city === property.city && button.dataset.street === property.streetName && button.dataset.number === property.streetNumber);
        if (isFavorite) {
            button.classList.add('active'); // Adaugă clasa "active" dacă proprietatea este favorite
        } else {
            button.classList.remove('active'); // Altfel, elimină clasa "active"
...
function removeFromFavorites(city, streetName, streetNumber) {
    ...
    tableRows.forEach(row => {
        let cityCell = row.cells[0]; // Prima celulă din rând este cea care conține orașul
        if (cityCell.textContent.trim() === city && row.getAttribute('data-favorite') === 'true') { // Verifică dacă orașul din rând corespunde orașului specificat și dacă rândul este în lista de favorite
            row.remove(); // Șterge rândul din tabel
            let loginUser = JSON.parse(localStorage.getItem("Log")) || {}; // Obține utilizatorul curent din local storage
            let userFavoritesKey = 'favorites_' + loginUser.email; // Creează cheia specifică utilizatorului pentru lista de favorite
            let favorites = JSON.parse(localStorage.getItem(userFavoritesKey)) || []; // Obține lista de proprietăți favorite specifică utilizatorului din local storage
            // Folosind orașul, numele străzii și numărul de stradă pentru a filtra proprietatea și a o elimina din lista de favorite
            let updatedFavorites = favorites.filter(property => !(property.city === city && property.streetName === streetName && property.streetNumber === streetNumber)); 
            localStorage.setItem(userFavoritesKey, JSON.stringify(updatedFavorites)); // Actualizăm lista de favorite în localStorage
            ...
            if (document.querySelector('.tabel') && document.querySelector('.tabel').style.display === 'block') {
